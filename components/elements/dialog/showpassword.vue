...
<template>
   ...
   <div class="dialog-card">
     ...
     <div class="card-body">
       ...
       <p v-if="$props.modalType" :class="`${useMapData().getStyleColor($props.modalType)}`">Password: {{ $props.modalText }}</p>
     </div>
     ...
     <div class="card-footer">
       ...
       <button v-if="$props.modalType"  :class="`${useMapData().getStyleButtonColor($props.modalType)}`" type="button" @click="onConfirmModal()">ปิด</button>
     </div>
   </div>
 </dialog>
</template>
