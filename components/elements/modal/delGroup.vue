...
<template>
   ...
   <div class="dialog-card">
     ...
     <div class="card-header">
       <button class="btn btn-close btn-close-modal" @click="hiddenDialogModal()">ปิด </button>
     </div>
     <div class="card-body">
     ...
     <div class="card-footer">
       <button v-if="$props.modalType"  :class="`${useMapData().getStyleButtonColor($props.modalType)}`" type="button" @click="onConfirmModal()">ใช่ ยืนยันการลบ</button>
       <button class="btn-gray btn-cancel-modal" type="button" @click="hiddenDialogModal()">ยกเลิก</button>
     </div>
   </div>
 </dialog>
</template>
