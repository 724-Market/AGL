<template>
  ...
  const state = useStoreStateOrder()
    ...
    if (AuthenInfo.value) {
      ...
      if(setOrderNo){
            orderGetInfo.value = setOrderNo.value
      } 

      const paymentNO = Array.isArray(route.params.id) ? route.params.id[0] : route.params.id ?? '';
      ...
      if (status.value == "Success") {
        ...
        if (
          ...
          response.apiResponse.Data
            ...
            console.log("else "+response.apiResponse.Data.Order[0].OrderNo)
          paymentGetInfo.value = response.apiResponse.Data.Payment[0];
          orderGetInfo.value = response.apiResponse.Data.Order[0].OrderNo;
          status.value =
            paymentGetInfo.value.IsSuccess && !paymentGetInfo.value.IsDelete
              ? "Success"
              : "Cancel";
