          ...
          <div class="card">
            <div class="card-body">
              ...
              <section class="surveys">
                ...
                <div class="survey-item"
                  v-if="value && value.isNonLifeLicense === 'notHasNonLifeLicense' && !recommender">
                  <header class="question">
                    <h3 class="topic">มีผู้แนะนำในการสมัครสมาชิกหรือไม่?</h3>
                    <p class="hint">รหัสผู้แนะนำขึ้นต้นด้วย AM</p>
                  </header>
                  <div class="answer">
                    <div class="choice">
                      <FormKit type="togglebuttons" name="isRecommender" enforced :options="{
                        'notHasRecommender': 'ไม่มีผู้แนะนำ',
                        'hasRecommender': 'มีผู้แนะนำ',
                      }" validation="required" :validation-messages="{ required: 'กรุณาเลือกข้อมูล' }" />
                    </div>
                    <div class="option" v-if="value && value.isRecommender === 'hasRecommender'">
                      <ElementsFormAgentCode />
                      <FormKit type="button" label="ตรวจสอบสถานะ" name="agent-check-submit" :classes="{
                        input: 'btn-primary',
                        outer: 'form-actions',
...
const recommender = ref(route.query.ref || '')
